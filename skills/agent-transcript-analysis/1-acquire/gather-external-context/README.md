...
Phase 1's context-gathering step. Pulls the ticket, the pull request, and the user's background into one `external-context.json` that travels with the transcript.
...
`get-claude-code-transcript-from-local` gives you the *what*. This skill gathers the *why*, once, up front, so phases 2-4 never have to.
...
- **Gather once, transcript-wide.** One up-front pull beats every analyzer re-deriving context per Segment. The narrow, per-Segment counterpart is `pull-together-goal-context` in phase 3 — it reaches out only when a specific Segment's Goal is still unclear.
