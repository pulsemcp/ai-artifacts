...
description: >
  ...
  travels with the transcript through every later phase. Use after
...
This skill closes that gap. Once per transcript, it pulls the surrounding context from the systems that hold it and consolidates it into a single `external-context.json` that rides in `tmp_dir` alongside `transcript.json` for phases 2-4 to read.
...
- [ ] Point the user at `review-external-context` so low-confidence inferences get a human check before phase 2
...
- Per-Segment context lookups during analysis — that's `pull-together-goal-context` in phase 3, which does narrow, on-demand pulls once a specific Segment's Goal is unclear. This skill does the one-time, transcript-wide gather up front.
