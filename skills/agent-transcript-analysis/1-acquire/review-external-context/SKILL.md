---
name: review-external-context
description: >
  Open a local browser UI to audit and correct the external-context.json
  drafted by gather-external-context. Resolving the right ticket, PR, and
...
`gather-external-context` emits `external-context.json` — an **inferred draft**. Matching a transcript to its ticket, its PR, and the user's background means reading signals (branch names, URLs in messages, repo slugs) and guessing — and some of those guesses will be wrong.
...
- [ ] Confirm `tmp_dir` contains `external-context.json` (or `external-context.reviewed.json`); if not, run `gather-external-context` first
...
- Producing the draft — that's `gather-external-context`.
- Acting on the corrections to improve `gather-external-context` — surface that as a flagged opportunity for the user; this skill does not edit any skill, and there is no automated learn-from loop for context (yet).
