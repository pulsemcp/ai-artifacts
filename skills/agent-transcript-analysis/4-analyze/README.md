...
The labeling layer. Five sibling buckets, each turning transcripts and Segments into structured findings. Four are per-Segment and driven by the orchestrator (Tier 3); the fifth, `analyze-cross-transcript/`, works across many transcripts' reports and runs separately. Two further skills sit at the tier root — `review-analysis` and `learn-from-analysis-corrections` — forming a human-in-the-loop review loop over whatever findings the buckets emit.
...
The five buckets emit AI *drafts*. Two skills at this tier's root close a human-in-the-loop loop over them — they are not buckets, so they sit at `4-analyze/` root rather than inside one:
...
`review-analysis` is parametrised by `kind` and reviews any of the five findings lists (the four per-Segment buckets plus `analyze-cross-transcript`). This pair mirrors tier 2's `review-transcript-segments` / `learn-from-segment-corrections`, pointed at the tier-4 findings instead of the Segment tree.
...
- **The review loop lives at the tier root, not in a bucket.** `review-analysis` and `learn-from-analysis-corrections` are bucket-agnostic — `review-analysis` reviews any `findings.<kind>.json` regardless of which analyzer drafted it. Putting them inside a bucket would falsely imply they belong to it. They sit at `4-analyze/` root for the same reason `review-transcript-segments` sits at `2-decompose/` root: a review checkpoint is a sibling of the thing it reviews, not a child of it.
