...
Upstream: `3-orchestrate/analyze-agent-transcript` runs this once per Segment whose `trigger.kind == "New" && trigger.source == "user"` — the case formerly known as an Initial Prompt.
Downstream: `prompting_recommendation` flows into the Prompting bucket of the final report. `deterministic_trigger_candidate` findings are also visible to `analyze-mcp-gaps` when the trigger implies an MCP server (e.g. listening to GitHub events).
