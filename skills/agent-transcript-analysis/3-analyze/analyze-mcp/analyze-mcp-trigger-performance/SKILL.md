...
- [ ] Inventory MCP tool calls in the segment. For each, was it the right call at the right time?
- [ ] Walk the segment for moments where the agent did something an available MCP tool was designed for, but reached for a CLI or wrote raw HTTP / SDK calls instead
