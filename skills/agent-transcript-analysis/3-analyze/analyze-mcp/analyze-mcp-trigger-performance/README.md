...
Invoked once per segment by `analyze-agent-transcript`. MCP-side counterpart to `analyze-skill-trigger-performance`. Companion to `analyze-mcp-action-performance` (the behavior) and `analyze-mcp-gaps` (the missing).
...
- **Skill-wrapped misses go elsewhere.** If a Skill was supposed to invoke the tool and didn't, that's a Skill-body finding (`analyze-skill-action-performance`), not an MCP trigger finding.
