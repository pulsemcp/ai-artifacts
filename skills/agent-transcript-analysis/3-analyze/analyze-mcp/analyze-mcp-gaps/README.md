...
Invoked once per segment by `analyze-agent-transcript`. Companion to `analyze-mcp-trigger-performance` and `analyze-mcp-action-performance`, which both work on tools that *do* exist.
...
- **Expect a paired Skill proposal.** Closing a closed-loop gap typically needs both an MCP server (for the capability) and a Skill (for the orchestration). `analyze-skill-gaps` usually proposes the companion.
