...
`analyze-agent-transcript` runs all three per segment. Same trigger / action / gaps split as the Skills bucket; the same boundary rules apply (a wrong response from the right tool is an *action* problem, not a trigger problem).
