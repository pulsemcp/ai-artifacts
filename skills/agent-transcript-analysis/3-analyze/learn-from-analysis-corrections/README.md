...
The feedback half of the tier-3 review loop. Reads the human corrections captured by `review-analysis` and **flags concrete improvement opportunities** for the tier-3 analyzers — it does not edit any skill.
...
Review without this skill: a one-shot cleanup. Review with it: a standing read on which tier-3 analyzers need work.
...
The output is a **write-up of flagged opportunities for a human**, not an applied change. This skill never edits the tier-3 analyzers — the skill files it can see are a deployed copy, not the source of truth. It does the analysis that makes a change obvious and well-motivated; a human makes the change, at its source, through the usual PR gate.
...
learn-from-analysis-corrections   →  flagged opportunities for the tier-3 analyzers
        └────────────────── close the loop ──────────────────┘
```
...
- **Stays in its lane.** A correction on a `skills` finding is attributed to the `analyze-skill-*` analyzers, a `prompts` correction to the `analyze-prompt-*` analyzers, and so on — never cross-attributed. This skill only flags opportunities in the *tier-3 analyzers*; decomposer corrections are `learn-from-segment-corrections`'s job.

## Mirrors the tier-2 loop

This skill is the tier-3 twin of `learn-from-segment-corrections`. Same shape — read a structured correction log, cluster it, flag opportunities, never apply — pointed at the tier-3 analyzers instead of the decomposer. The two never overlap: tier-2 corrections are about *where the Segment tree was wrong*, tier-3 corrections are about *where a conclusion drawn from that tree was wrong*.
