...
Upstream: `3-analyze/analyze-agent-transcript` runs this for every Segment whose Outcome is Failure, plus every Segment whose next sibling opens with a Correction trigger (retro-Failure — either user-source or agent-source).
Downstream: the `recommendation_seed` is handed to the matching gap analyzer (`analyze-agent-transcript-skill-gaps`, `analyze-agent-transcript-mcp-gaps`) or to `analyze-agent-transcript-user-prompt`, which fleshes it into a real proposal.
