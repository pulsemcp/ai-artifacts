...
Output from this bucket *does not* directly map to one of the final report's three buckets — instead, each finding declares, via a `recommendation_route`, which downstream bucket(s) (Prompting / Skills / MCP) its recommendation flows to. `synthesize-report` (tier 4) follows that route when it folds the findings into the report.
