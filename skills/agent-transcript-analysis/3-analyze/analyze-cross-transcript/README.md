...
Tier 3's cross-cutting bucket. The other three artifact buckets and `analyze-outcomes` work *within* one transcript, per Segment; this bucket works *across* many transcripts' per-transcript `findings.*.json` sets. Single-transcript analysis catches issues inside a session — this bucket catches habits across sessions.
...
- **Pure aggregation, no re-walking.** This bucket reads only the structured outputs of per-transcript analysis — the `findings.*.json` sets (and the `segments.json` they were derived from), never the synthesized `report.md` and never raw JSONL. The report is already filtered and synthesized; running cross-transcript analysis on reports would miss the individually-minor findings that only matter once they recur. If something is missing from a `segments.json`, fix tier 2 and re-run the lower tiers; don't paper over it here.
