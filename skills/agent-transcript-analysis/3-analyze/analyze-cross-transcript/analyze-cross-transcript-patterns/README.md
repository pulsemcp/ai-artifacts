...
The labeling skill of tier 3's `analyze-cross-transcript` bucket. Operates on the consolidated reports of many already-analyzed transcripts — the across-sessions counterpart to the per-Segment analyzers in the other tier-3 buckets.

## How it plugs in

Upstream: a batch of single-transcript reports produced by `analyze-agent-transcript`.
Downstream: its findings feed `synthesize-report` in tier 4 — the same as every other tier-3 bucket. This skill produces *findings* ("this pattern recurs in N sessions"); turning them into a prioritized change list is `synthesize-report`'s job.
