---
name: review-agent-transcript-analysis
description: >
  ...
  analyze-cross-agent-transcript-patterns) and before learn-from-agent-transcript-analysis-corrections.
...
This skill puts one bucket's draft in front of a human in an editable UI — one finding at a time, thumbs-up / correct / reject — and records every correction with enough provenance that `learn-from-agent-transcript-analysis-corrections` can turn the fixes into flagged improvement opportunities for the analyzer that drafted them.

It is the **review checkpoint for phase 3**, the phase-3 counterpart to `review-agent-transcript-segments` (phase 2) and `review-agent-transcript-external-context` (phase 1). It is optional — `analyze-agent-transcript` consumes `findings.<kind>.json` fine on its own — but every correction captured here makes the next analysis better.
...
- `kind` (required): which findings bucket — one of `outcomes`, `prompts`, `skills`, `mcp`, `cross-transcript`. `analyze-agent-transcript` writes the first four; `analyze-cross-agent-transcript-patterns` writes `cross-transcript`.
...
- [ ] Confirm `tmp_dir` contains `findings.<kind>.json` (or `findings.<kind>.reviewed.json`); if not, run the analyzers first — `analyze-agent-transcript` for `outcomes` / `prompts` / `skills` / `mcp`, `analyze-cross-agent-transcript-patterns` for `cross-transcript`
...
The correction-provenance contract lives in the bundled `review.py`, the same contract `learn-from-agent-transcript-analysis-corrections` reads. `review_server.py`, `review_ui.html`, and `review.py` are bundled self-contained copies in this skill folder — parametrised only by `kind`, so a new reviewable `kind` needs no new code.

## Out of scope

- Producing the draft — that's the phase-3 analyzers, driven by `analyze-agent-transcript` (or `analyze-cross-agent-transcript-patterns`).
- Turning the corrections into improvement opportunities for the analyzers — that's the sibling skill `learn-from-agent-transcript-analysis-corrections`.
- Reviewing the Segment tree (phase 2) or the external-context bundle (phase 1) — those are `review-agent-transcript-segments` and `review-agent-transcript-external-context`.
