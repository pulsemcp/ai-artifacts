...
Invoked once per segment by `analyze-agent-transcript`. Companion to `analyze-skill-trigger-performance` and `analyze-skill-action-performance`, which both work on Skills that *do* exist.
