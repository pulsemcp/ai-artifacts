...
Invoked once per segment by `analyze-agent-transcript`. Companion to `analyze-agent-transcript-skill-trigger-performance` (the description) and `analyze-agent-transcript-skill-gaps` (the missing).
