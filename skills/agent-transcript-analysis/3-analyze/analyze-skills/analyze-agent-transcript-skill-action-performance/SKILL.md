---
name: analyze-agent-transcript-skill-action-performance
description: >
...
- **A `modify` here is the *owner* of any defect in an existing Skill's body or shape.** When a Skill fired and the fix is to its body, this analyzer's `modify` recommendation is the canonical finding — `analyze-agent-transcript-skill-gaps` must **not** also propose a new Skill for the same defect (it defers to this finding). If a failure-hypothesis seed pointed `analyze-agent-transcript-skill-gaps` at the same defect, expect the gap analyzer to record the deferral and let `synthesize-agent-transcript-analysis-report` reconcile to this `modify`.
