---
name: analyze-agent-transcript-prompt-ambition
description: >
...
- **Stay in the prompting bucket.** This skill does not recommend Skills or MCP servers directly; if a deterministic trigger candidate implies an MCP server is needed, hand the seed to `analyze-agent-transcript-mcp-gaps` via the orchestrator.
