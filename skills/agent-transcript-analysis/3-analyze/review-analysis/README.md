...
The acquisition steps that just *move* data (`find-all-claude-code-transcripts-on-local`, `get-claude-code-transcript-from-local`) are deterministic — no review UI needed. But every *interpretive* step gets a review checkpoint: `review-external-context` for the gathered context (phase 1), `review-transcript-segments` for the Segment tree (phase 2), and **this skill for the analyzer findings** (phase 3).
