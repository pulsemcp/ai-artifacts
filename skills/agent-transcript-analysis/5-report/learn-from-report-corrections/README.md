...
The feedback half of the tier-5 review loop. Reads the human corrections captured by `review-report` and **flags concrete improvement opportunities** for `synthesize-report` — it does not edit any skill.
...
- **One skill, many behaviors.** Every tier-5 correction traces back to one skill — `synthesize-report` — so the diagnostic question is not *which skill* (as it is for the tier-4 learner) but *which behavior*: over-reaching the findings, mis-routing into the wrong bucket, mis-calibrating priority, or a weak philosophy check.

## Mirrors the tier-2 and tier-4 loops

This skill is the tier-5 member of the `learn-from-*-corrections` family — `learn-from-segment-corrections` (tier 2), `learn-from-analysis-corrections` (tier 4), `learn-from-report-corrections` (tier 5). Same shape — read a structured correction log, cluster it, flag opportunities, never apply — each pointed at a different interpretive step. The three never overlap: a wrong Segment tree is tier 2's, a wrong finding drawn from a correct tree is tier 4's, and a wrong recommendation synthesized from correct findings is tier 5's.
