...
Tier 5's synthesis skill. Reads the tier-4 findings for a transcript and produces the consolidated, actionable recommendation slate — `findings.report.json` (reviewable) and `report.md` (human-readable).

## Why this exists

Tier 4 ends with **labels**: flat lists of conclusions, one file per bucket. Nobody can act on a label. Someone has to cluster the findings, route them into Prompting / Skills / MCP, cross-check them against team philosophy, prioritize them, and write the result down as next steps. That work — the **leap from analysis to recommendations** — is its own interpretive step, and the pipeline gives it its own tier so the leap is visible, reviewable, and improvable.

Before tier 5 existed, this synthesis was buried inside the tier-3 orchestrator. Pulling it out means the orchestrator just *drives* (decompose → fan out → write findings) and the report is a first-class artifact with its own review checkpoint — exactly the shape tiers 2 and 4 already have.
...
It has no `main.py`: the work is reading findings JSON and writing a report, which the agent does directly — the same shape as the tier-4 analyzers and `learn-from-analysis-corrections`.
...
`findings.report.json` is the **same envelope as every tier-4 findings file** — `{kind, items: [{id, …}]}` — with `kind: "report"`. That is deliberate: `review-report` reviews it with the exact same engine (`review.py` + `review_server.py` + `review_ui.html`) that `review-analysis` uses for tier-4 findings. `REPORT_KIND` was reserved in that engine from the start for precisely this.

Each item is one recommendation: `bucket` (prompting / skills / mcp), `action` (create / modify / delete / adopt / stop), `title`, `recommendation`, `rationale`, `sources` (the tier-4 finding ids it was synthesized from), `priority`, `effort`, `philosophy_check`. The `sources` list is what makes the leap auditable — `review-report` checks each recommendation against the findings it claims to follow from.
...
Same shape as tier 2 (`decompose` → `review-transcript-segments` → `learn-from-segment-corrections`) and tier 4 (`analyze-*` → `review-analysis` → `learn-from-analysis-corrections`).

## Design decisions

- **The synthesis is its own tier.** Labeling (tier 4) and synthesis (tier 5) are different kinds of work, and the leap between them is exactly the kind of interpretive step the plugin gives a review checkpoint. Burying it in the orchestrator hid it from review.
- **Same envelope as tier-4 findings.** `findings.report.json` is `{kind, items}` with `kind: "report"`, so the review subsystem reviews it with no new code — `review-report` is a thin wrapper over the shared engine, not a second UI.
