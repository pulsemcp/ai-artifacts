...
name: review-report
description: >
  Open a local browser UI to audit and correct the AI-synthesized tier-5
  report — findings.report.json, the consolidated recommendation slate
  synthesize-report produces from the tier-4 findings. The report is a draft:
...
It is the **review checkpoint for tier 5**, the tier-5 counterpart to `review-transcript-segments` (tier 2) and `review-analysis` (tier 4). It is optional — `findings.report.json` stands on its own — but every correction captured here makes the next synthesis better. Crucially, this is where a human reviews **the leap itself**: does each recommendation actually follow from the findings in its `sources` list?
...
`findings.report.json` is the **same envelope as every tier-4 findings file** — the review subsystem is deliberately schema-agnostic about it:
...
- [ ] The user audits each recommendation against its `sources`: does this recommendation actually follow from the tier-4 findings it cites? Is the `bucket` right? the `priority`? does `philosophy_check` hold up?
...
The correction-provenance contract lives in the bundled `review.py`, the same contract `learn-from-report-corrections` reads. The engine is `kind`-parametrised: `review-analysis` runs it with a tier-4 `kind`, this skill runs it with `report`. No new server, no new UI — only the `kind` differs.
...
- Reviewing the tier-4 findings, the Segment tree, or the external-context bundle — those are `review-analysis`, `review-transcript-segments`, and `review-external-context`.
