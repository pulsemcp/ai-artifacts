...
- **`findings.report.json`** — the **reviewable recommendation slate**. Same envelope as every phase-3 findings file (`{kind, items: [{id, …}]}`) with `kind: "report"`, so `review-agent-transcript-analysis-report` reviews it with the exact same engine `review-agent-transcript-analysis` uses for phase-3 findings. Each item is one recommendation:
...
- [ ] `review-agent-transcript-analysis-report` is the canonical human checkpoint over the recommendation slate, run when the user wants to correct/reject items in `findings.report.json`. **Do not** end the report itself with a pointer at `review-agent-transcript-analysis-report` — that's a CLI hint that doesn't earn its slot on a navigable report page.
...
- Human review of the report — that's `review-agent-transcript-analysis-report`, the phase-4 checkpoint over `findings.report.json` in `batch_dir`.
- Turning review corrections into improvements — that's `learn-from-agent-transcript-analysis-report-corrections`.
...
- **Same envelope as phase 3, on purpose.** `findings.report.json` is `{kind, items}` so the review subsystem (`review.py` + `review_server.py` + `review_ui.html`) reviews it unchanged — `review-agent-transcript-analysis-report` is a thin wrapper, not a new UI.
