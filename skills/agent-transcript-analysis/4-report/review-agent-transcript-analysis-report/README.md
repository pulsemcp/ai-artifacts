# `review-agent-transcript-analysis-report`
...
So `findings.report.json` is treated as a **draft** — the same stance `review-agent-transcript-segments` takes toward the Segment tree and `review-agent-transcript-analysis` takes toward the phase-3 findings. This skill is where a human reviews the leap itself: for each recommendation, *does this actually follow from the findings it cites?* Their corrections are captured in a structured, replayable form so `synthesize-agent-transcript-analysis-report` can be improved.
...
`review_server.py`, `review_ui.html`, and `review.py` are **byte-identical copies** of the engine `review-agent-transcript-analysis` bundles. That engine was `kind`-parametrised and reserved `REPORT_KIND` from the start, precisely so phase 4's report would review with no new code. They are bundled (not imported from a sibling) because a skill running from a deployed `.claude/skills/` directory can't reach another skill's files — portability beats deduplication.

## The reviewed sibling

```
batch_dir/
  findings.report.json            # synthesize-agent-transcript-analysis-report draft — never touched
...
```
synthesize-agent-transcript-analysis-report             →  findings.report.json (AI draft)
...
- **The draft is immutable; the review is a sibling.** Keeping `findings.report.json` pristine means the synthesis-vs-human diff is always inspectable, and a bad review can be thrown away by deleting one file. Same contract as `review-agent-transcript-analysis` and `review-agent-transcript-segments`.
- **One engine, every reviewable kind.** `findings.report.json` is the same `{kind, items}` envelope as a phase-3 findings file, so the server, UI, and provenance contract are reused unchanged — parametrised only by `kind`. `review-agent-transcript-analysis` runs the engine with a phase-3 `kind`; this skill runs it with `report`. `REPORT_KIND` was reserved in `review.py` from the start for exactly this.
...
- **Review the leap, not the labels.** Phase 3's review (`review-agent-transcript-analysis`) checks whether a *finding* is true. This skill checks whether a *recommendation* follows from the findings it cites — a different question, which is why phase 4 gets its own checkpoint instead of folding into phase 3's.
...
This skill is the phase-4 member of a family: `review-agent-transcript-external-context` (phase 1), `review-agent-transcript-segments` (phase 2), `review-agent-transcript-analysis` (phase 3), `review-agent-transcript-analysis-report` (phase 4). Every interpretive step in the pipeline gets a human checkpoint with the same contract — an immutable draft, a reviewed sibling, an append-only correction log, and a `learn-from-*-corrections` sibling that turns the log into flagged improvement opportunities.
