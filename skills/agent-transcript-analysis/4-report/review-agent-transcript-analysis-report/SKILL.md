---
name: review-agent-transcript-analysis-report
description: >
  ...
  recommendation slate synthesize-agent-transcript-analysis-report produces from the whole batch's
  ...
  overwritten) with full correction provenance. Use after synthesize-agent-transcript-analysis-report
  and before learn-from-agent-transcript-analysis-report-corrections. Optional but recommended human
...
`synthesize-agent-transcript-analysis-report` makes the pipeline's one **leap from analysis to recommendations** — it reads the whole batch's phase-3 findings and synthesizes them into `findings.report.json`, a flat slate of actionable next steps. That leap is interpretive: a recommendation can over-reach what its findings actually support, mis-prioritize, mis-route a finding into the wrong bucket, or contradict team philosophy. The synthesis will get some of them wrong.

This skill puts the report in front of a human in an editable UI — one recommendation at a time, thumbs-up / correct / reject — and records every correction with enough provenance that `learn-from-agent-transcript-analysis-report-corrections` can turn the fixes into flagged improvement opportunities for `synthesize-agent-transcript-analysis-report`.

It is the **review checkpoint for phase 4**, the phase-4 counterpart to `review-agent-transcript-segments` (phase 2) and `review-agent-transcript-analysis` (phase 3). It is optional — `findings.report.json` stands on its own — but every correction captured here makes the next synthesis better. Crucially, this is where a human reviews **the leap itself**: does each recommendation actually follow from the findings in its `sources` list?

## Inputs

- `batch_dir` (required): the batch-level working directory containing `findings.report.json` (or an existing `findings.report.reviewed.json` to keep iterating), produced by `synthesize-agent-transcript-analysis-report`.
...
The only guarantees the reviewer (and the bundled `review.py`) rely on: a `kind`, an `items` list, and a unique non-empty `id` on every item. The recommendation fields — `bucket`, `action`, `title`, `recommendation`, `rationale`, `sources`, `priority`, `effort`, `philosophy_check` — are `synthesize-agent-transcript-analysis-report`'s business; the UI renders them generically.
...
The correction-provenance contract lives in the bundled `review.py`, the same contract `learn-from-agent-transcript-analysis-report-corrections` reads. The engine is `kind`-parametrised: `review-agent-transcript-analysis` runs it with a phase-3 `kind`, this skill runs it with `report`. No new server, no new UI — only the `kind` differs.

## Out of scope

- Producing the draft — that's `synthesize-agent-transcript-analysis-report`.
- Turning the corrections into improvement opportunities for `synthesize-agent-transcript-analysis-report` — that's the sibling skill `learn-from-agent-transcript-analysis-report-corrections`.
- Reviewing the phase-3 findings, the Segment tree, or the external-context bundle — those are `review-agent-transcript-analysis`, `review-agent-transcript-segments`, and `review-agent-transcript-external-context`.
- Re-running the synthesis — a rejected recommendation is left in place and stamped, not regenerated. Re-run `synthesize-agent-transcript-analysis-report` for a fresh draft.
