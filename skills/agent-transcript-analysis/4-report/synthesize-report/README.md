...
It has no `main.py`: the work is reading findings JSON and writing a report, which the agent does directly — the same shape as the phase-3 analyzers and `learn-from-analysis-corrections`.
...
Same shape as phase 2 (`decompose` → `review-transcript-segments` → `learn-from-segment-corrections`) and phase 3 (`analyze-*` → `review-analysis` → `learn-from-analysis-corrections`).
...
- **Same envelope as phase-3 findings.** `findings.report.json` is `{kind, items}` with `kind: "report"`, so the review subsystem reviews it with no new code — `review-report` is a thin wrapper over the shared engine, not a second UI.
