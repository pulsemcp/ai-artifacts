# `learn-from-agent-transcript-analysis-report-corrections`

The feedback half of the phase-4 review loop. Reads the human corrections captured by `review-agent-transcript-analysis-report` and **flags concrete improvement opportunities** for `synthesize-agent-transcript-analysis-report` — it does not edit any skill.

## Why this exists

`review-agent-transcript-analysis-report` is worth running on its own — it gives you a human-blessed `findings.report.reviewed.json`. But the corrections it captures are also *signal*: they are a precise record of where the synthesis's leap from findings to recommendations diverged from a human's judgment. This skill closes the loop — it turns that record into a clear, well-evidenced write-up of where `synthesize-agent-transcript-analysis-report` is drifting, so a human can decide what to change.
...
It has no `main.py`: the work is reading correction logs and writing a proposal, which the agent does directly. The correction-provenance contract it reads is the one `review-agent-transcript-analysis-report` writes — defined in that skill's bundled `review.py`.
...
The output is a **write-up of flagged opportunities for a human**, not an applied change. This skill never edits `synthesize-agent-transcript-analysis-report` — the skill files it can see are a deployed copy, not the source of truth. It does the analysis that makes a change obvious and well-motivated; a human makes the change, at its source, through the usual PR gate.
...
- **Flag, don't apply.** The same human-in-the-loop principle that makes `findings.report.json` a draft makes this skill's output a flag, not a fix. A skill that rewrote `synthesize-agent-transcript-analysis-report` from a handful of corrections would be unreviewable — and the skill files it sees at runtime are a deployed copy anyway, not the source of truth.
