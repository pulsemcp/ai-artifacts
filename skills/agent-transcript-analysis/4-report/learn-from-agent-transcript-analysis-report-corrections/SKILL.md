---
name: learn-from-agent-transcript-analysis-report-corrections
description: >
  Read the human corrections captured in one or more
  findings.report.reviewed.json files (produced by review-agent-transcript-analysis-report), cluster them
  into recurring patterns, and surface them as flagged improvement
  opportunities for synthesize-agent-transcript-analysis-report — the skill that makes the leap from
  ...
  few reports have been reviewed. Optional sibling of review-agent-transcript-analysis-report.
user-invocable: true
---

# Learn from report corrections

`review-agent-transcript-analysis-report` captures, in structured form, every place a human disagreed with `synthesize-agent-transcript-analysis-report`'s leap from findings to recommendations. This skill is the **other half of that loop**: it reads those corrections, finds the patterns in them, and **flags concrete improvement opportunities** for `synthesize-agent-transcript-analysis-report` — so a human knows exactly where the synthesis is drifting from what they'd have recommended.

It **flags opportunities; it does not apply them.** This skill never edits `synthesize-agent-transcript-analysis-report` (or any other skill). The skill files visible at runtime are a deployed copy — their source of truth lives elsewhere — so the right move is always to *surface* the opportunity for a human, not to patch the copy in place.
...
- One or more `batch_dir`s, each containing a `findings.report.reviewed.json` (the output of `review-agent-transcript-analysis-report` over that batch's report). The more reviewed reports, the stronger the pattern signal — a single rejection is an anecdote; the same kind of rejection across five batch reports is a heuristic.
...
- **`report-correction-learnings.md`** — written to the first `batch_dir` (or a path the caller specifies). A write-up of flagged opportunities for a human, not an applied change. It contains:
  ...
  - **Flagged opportunities** — for each pattern, **how `synthesize-agent-transcript-analysis-report` appears to be drifting** and **in which direction** (over-reaching the findings / mis-routing into the wrong bucket / over-prioritizing / weak `philosophy_check`), with the corrections that motivate it cited as evidence. Describe the opportunity precisely enough that a human can act on it — but stop there: don't write a ready-to-paste edit, and don't point at skill files by path.
  - **Open questions** — corrections that don't generalize yet, or that conflict with each other, flagged for a human to weigh in.

This skill **flags; it does not apply.** Whoever picks up the write-up decides whether and how to change `synthesize-agent-transcript-analysis-report`, and makes that change at its source of truth through the normal PR gate.
...
- [ ] **Trace each cluster to a synthesis behavior.** Unlike the phase-3 learner, every correction here points back at one skill — `synthesize-agent-transcript-analysis-report` — so the question is not *which* skill but *which behavior*:
...
- **One skill, many behaviors.** Every correction here is about `synthesize-agent-transcript-analysis-report` — the diagnostic work is naming *which behavior* drifted, not *which skill*.

## Out of scope

- Capturing corrections — that's `review-agent-transcript-analysis-report`.
- Editing `synthesize-agent-transcript-analysis-report` or any other skill — this skill **flags opportunities for a human**; it never patches a skill. The deployed skill files are a copy; changes belong at the source of truth, behind the normal PR gate.
- Analyzer corrections (phase 3) and decomposer corrections (phase 2) — those are `learn-from-agent-transcript-analysis-corrections` and `learn-from-agent-transcript-segment-corrections`. This skill is narrowly about what the phase-4 *synthesis* review history reveals: a phase-3 finding being wrong is the analyzer's problem; a correct finding being turned into a bad recommendation is `synthesize-agent-transcript-analysis-report`'s.

## Notes

- This skill is **only as good as the review volume**. Run it after several reports have been through `review-agent-transcript-analysis-report`, not after one.
- Conflicting corrections are signal too: if one reviewer rejects a recommendation another approves, the synthesis may be making a genuinely ambiguous call — that belongs in "Open questions", and is worth flagging as a possible gap in `synthesize-agent-transcript-analysis-report`'s own contract rather than a tuning issue.
- This skill is the phase-4 twin of `learn-from-agent-transcript-analysis-corrections` (phase 3) and `learn-from-agent-transcript-segment-corrections` (phase 2). Same shape — read a structured correction log, cluster it, flag opportunities, never apply — pointed at the synthesis step instead of the analyzers or the decomposer.
