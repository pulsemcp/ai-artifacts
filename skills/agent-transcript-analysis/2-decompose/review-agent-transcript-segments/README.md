...
`segment_review.py` is a bundled self-contained module in this skill folder (not a shared library import) so the skill runs from a deployed `.claude/skills/` directory. It defines the correction-provenance contract that `learn-from-agent-transcript-segment-corrections` also reads.
...
- **The log is the source of truth for provenance.** `write_reviewed` strips stale `review` stamps and re-derives them from the log on every save — so the stamps can never drift from the log, and the log alone is enough for `learn-from-agent-transcript-segment-corrections` to work.
