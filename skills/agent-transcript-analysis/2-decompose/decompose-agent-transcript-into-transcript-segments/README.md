...
Downstream: every phase-3 skill reads `segments.json` from the tmp folder, and dereferences event ids back into `transcript.json` for evidence. The flamegraph is for humans only.

## Design decisions

- **Walk the Transcript once.** This is the only skill that walks every event. Cost lives here so analyzers can stay cheap.
- **Read OpenTranscripts, not CC JSONL.** Phase 1 already normalized; this skill is vendor-agnostic by construction.
