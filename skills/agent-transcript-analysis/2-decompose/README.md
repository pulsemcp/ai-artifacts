...
Decomposition layer. Turns an OpenTranscripts `transcript.json` (produced by tier 1) into the recursive **Transcript Segment** tree that every downstream analyzer consumes.
...
Tier 1 → Tier 2 → Tier 3. The orchestrator and every per-segment analyzer in Tier 4 read `segments.json` and dereference event ids back into `transcript.json` — they never re-walk events from scratch.
