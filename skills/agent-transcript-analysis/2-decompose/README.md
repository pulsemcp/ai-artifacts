...
Decomposition layer. Turns an OpenTranscripts `transcript.json` (produced by phase 1) into the recursive **Transcript Segment** tree that every downstream analyzer consumes — and gives a human the chance to correct that tree before the analyzers run on it.
