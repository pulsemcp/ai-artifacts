# `learn-from-agent-transcript-segment-corrections`

The feedback half of the phase-2 review loop. Reads the human corrections captured by `review-agent-transcript-segments` and **flags concrete improvement opportunities** for `decompose-agent-transcript-into-transcript-segments` — it does not edit any skill.

## Why this exists

`review-agent-transcript-segments` is worth running on its own — it gives you a human-blessed `segments.reviewed.json`. But the corrections it captures are also *signal*: they are a precise record of where the AI decomposer's judgment diverged from a human's. This skill closes the loop — it turns that record into a clear, well-evidenced write-up of where the decomposer is drifting, so a human can decide what to change.
...
review-agent-transcript-segments           →  segments.reviewed.json + correction log
