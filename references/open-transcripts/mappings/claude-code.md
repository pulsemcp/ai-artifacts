...
How Claude Code's `~/.claude/projects/<project-slug>/<session-uuid>.jsonl` files map onto OpenTranscripts v0.1. This is the canonical field-by-field reference used by the `get-claude-code-transcript-from-local` skill.

Maintenance contract: this doc is the canonical spec for the mapping. When CC adds, renames, or removes a JSONL field, `get-claude-code-transcript-from-local` keeps running — any line it can't place becomes a `SystemEvent` and is accumulated in `provider.raw.unmapped_lines[]` rather than dropped — and the drift is reconciled back into this doc. Updating here is what promotes a drifted line back to a first-class event.
